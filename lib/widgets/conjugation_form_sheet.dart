...
Future<void> showConjugationSheet(
  ...
  return showFeedbackBottomSheet(
    context,
    builder: (context) => _ConjugationFormSheet(
      headword: headword,
...
class _ConjugationFormSheet extends ConsumerStatefulWidget {
  const _ConjugationFormSheet({this.headword, this.headwordId});

  final String? headword;
  final int? headwordId;

  @override
  ConsumerState<_ConjugationFormSheet> createState() =>
      _ConjugationFormSheetState();
}

class _ConjugationFormSheetState
    extends ConsumerState<_ConjugationFormSheet> {
  ...
  late List<String> _where;
  late List<String> _voice;
  late List<String> _person;
  ...
  void initState() {
    super.initState();
    final draft =
        ref.read(feedbackDraftServiceProvider).loadConjugationDraft();
    ...
    _where = List.from(draft.where);
    _voice = List.from(draft.voice);
    _person = List.from(draft.person);
  ...
  ConjugationDraft _currentDraft() => ConjugationDraft(
        name: _nameCtrl.text,
  ...
  void _saveDraft() {
    ref
        ...
        .saveConjugationDraft(_currentDraft());
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (isOnline) {
        ...
        final uri = buildConjugationEmailUri(
          name: _nameCtrl.text,
          email: _emailCtrl.text,
          headword: _headwordCtrl.text,
          problem: _problem,
      ...
      if (launched) {
        await ref
            .read(feedbackDraftServiceProvider)
            .clearConjugationTransientDraft();
                      ...
                      children: [
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                'Please help to improve these tables by correcting mistakes and adding missing conjugations.',
                                style: TextStyle(
                        ...
                        const SizedBox(height: 12),
                        FeedbackQuestionCard(
                          question: 'What is the Pāḷi headword?',
                          required: true,
                          child: TextFormField(
                        ...
                        FeedbackCheckboxCard(
                          ...
                          selected: _where,
                          onToggle: (v) => _toggleValue(_where, v),
                          submitted: _submitted,
                        ),
                        const SizedBox(height: 12),
                        FeedbackCheckboxCard(
                          question: 'Active or Reflexive?',
                          required: true,
                          options: const [
                            'Active (parassapada)',
                            'Reflexive (attanopada)',
                          ],
                          selected: _voice,
                          onToggle: (v) => _toggleValue(_voice, v),
                          submitted: _submitted,
                        ),
                        const SizedBox(height: 12),
                        FeedbackCheckboxCard(
                          ...
                          selected: _person,
                          onToggle: (v) => _toggleValue(_person, v),
                          submitted: _submitted,
                        ),
                        const SizedBox(height: 12),
                        FeedbackCheckboxCard(
